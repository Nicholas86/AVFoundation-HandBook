...
- (void)handleAudioFileStreamPackets:(const void *)packets
                     ...
                     numberOfPackets:(UInt32)numberOfPackets packetDescription:(AudioStreamPacketDescription *)packetDescriptions;

@end

#pragma mark - static callbacks
static void JKAudioFileStreamPropertyListener(void *inClientData,
                                              AudioFileStreamID inAudioFileStream,
                                              AudioFileStreamPropertyID inPropertyID,
                                              UInt32 *ioFlags) {
    JKAudioFileStream *audioFileStream = (__bridge JKAudioFileStream *) inClientData;
    [audioFileStream handleAudioFileStreamProperty:inPropertyID];
}

static void JKAudioFileStreamPacketsCallBack(void *inClientData,
...
#pragma mark -
@implementation JKAudioFileStream
...
- (instancetype)initWithFileType:(AudioFileTypeID)fileType
    ...
    if (self = [super init]) {
        ...
        _fileType = fileType;
        _fileSize = fileSize;
        [self _openAudioFileStreamWithFileTypeHint:_fileType
                                             error:error];
    }
    return self;
}
...
- (BOOL)_openAudioFileStreamWithFileTypeHint:(AudioFileTypeID)fileTypehint
                                       error:(NSError *__autoreleasing *)error {
...
- (void)handleAudioFileStreamPackets:(const void *)packets
    ...
    for (int i = 0; i < numberOfPackets; ++i)
    {
        ...
        JKParsedAudioData *parsedData = [JKParsedAudioData parsedAudioDataWithBytes:packets + packetOffset
                                                                  packetDescription:packetDescriptions[i]];
