...
const int JKAudioQueueBufferCount = 2;

@interface JKAdudioQueueBuffer : NSObject
@property (nonatomic,assign) AudioQueueBufferRef buffer;
@end
@implementation JKAdudioQueueBuffer
@end

@interface JKAudioOutputQueue () {
@private
...
@implementation JKAudioOutputQueue
...
- (void)_createAudioOutputQueue:(NSData *)magicCookie {
    OSStatus status = AudioQueueNewOutput(&_format,JKAudioQueueOutputCallback, (__bridge void *)(self), NULL, NULL, 0, &_audioQueue);
    if (status != noErr) {
        _audioQueue = NULL;
        return;
    }
    status = AudioQueueAddPropertyListener(_audioQueue, kAudioQueueProperty_IsRunning, JKAudioQueuePropertyCallback, (__bridge void *)(self));
    if (status != noErr)
    {
    ...
    if (_buffers.count == 0)
    {
        for (int i = 0; i < JKAudioQueueBufferCount; ++i)
        {
            ...
            JKAdudioQueueBuffer *bufferObj = [[JKAdudioQueueBuffer alloc] init];
...
- (BOOL)playData:(NSData *)data packetCount:(UInt32)packetCount packetDescriptions:(AudioStreamPacketDescription *)packetDescriptions isEof:(BOOL)isEof
{
    ...
    JKAdudioQueueBuffer *bufferObj = [_reusableBuffers firstObject];
    ...
    if (!bufferObj)
    {
        ...
        if (status == noErr)
        {
            bufferObj = [[JKAdudioQueueBuffer alloc] init];
            bufferObj.buffer = buffer;
        }
        else
        {
...
static void JKAudioQueueOutputCallback(void *inClientData, AudioQueueRef inAQ, AudioQueueBufferRef inBuffer) {
    JKAudioOutputQueue *audioOutputQueue = (__bridge JKAudioOutputQueue *)inClientData;
...
static void JKAudioQueuePropertyCallback(void *inUserData, AudioQueueRef inAQ, AudioQueuePropertyID inID) {
    JKAudioOutputQueue *audioQueue = (__bridge JKAudioOutputQueue *)inUserData;
