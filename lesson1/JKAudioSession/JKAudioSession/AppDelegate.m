...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(interruptionNotificationReceived:) name:JKAudioSessionInterruptionNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(routeChangeNotificationReceived:) name:JKAudioSessionRouteChangeNotification object:nil];
...
- (void)activeAudioSession {
    NSError *error = nil;
    if ([[JKAudioSession sharedInstance] setActive:YES error:&error])
    {
...
- (void)interruptionNotificationReceived:(NSNotification *)notification
{
    ...
    AudioSessionInterruptionType interruptionType = [notification.userInfo[JKAudioSessionInterruptionStateKey] unsignedIntValue];
...
{
    NSLog(@"route changed! %@",[JKAudioSession isAirplayActived] ? @"airplay actived" : @"airplay is actived");
    
    BOOL usingHeadset = [JKAudioSession usingHeadset];
    SInt32 routeChangeReason = [notification.userInfo[JKAudioSessionRouteChangeReason] intValue];
